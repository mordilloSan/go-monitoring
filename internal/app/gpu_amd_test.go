...
func TestUpdateAmdGpuDataWithFakeSysfs(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.InDelta(t, 25.0, gpu.Usage, 0.0001)
			assert.InDelta(t, tt.wantMemoryUsed, gpu.MemoryUsed, 0.0001)
			assert.InDelta(t, tt.wantMemoryTotal, gpu.MemoryTotal, 0.0001)
			assert.InDelta(t, 45.0, gpu.Temperature, 0.0001)
			assert.InDelta(t, 20.0, gpu.Power, 0.0001)
			assert.InDelta(t, 1.0, gpu.Count, 0.0001)
		})
	}
}

func TestLookupAmdgpuNameInFile(t *testing.T) {
	idsPath := filepath.Join("testdata", "amdgpu.ids")
...
func TestGetAmdGpuNameFromIdsFile(t *testing.T) {
	...
	// returns the expected name for our testdata file.
	idsPath := filepath.Join("testdata", "amdgpu.ids")
