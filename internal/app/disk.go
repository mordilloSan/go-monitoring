...
//
//nolint:gocognit // Filesystem registration has many device and mountpoint normalization branches.
...
func (m *fsManager) applyDiskIoCounters(cacheTimeMs uint16, systemStats *system.Stats, ioCounters map[string]disk.IOCountersStat, now time.Time) {
	...
	for name, d := range ioCounters {
		...
		if diskCounterReset(d, prev) {
			...
			stats.DiskIoStats = [6]float64{}
			if stats.Root {
				systemStats.DiskReadPs = 0
				systemStats.DiskWritePs = 0
				systemStats.DiskIO = [2]uint64{}
				systemStats.DiskIoStats = [6]float64{}
			}
			continue
		}
...
func diskCounterReset(d disk.IOCountersStat, prev prevDisk) bool {
	return d.ReadBytes < prev.readBytes ||
		d.WriteBytes < prev.writeBytes ||
		d.ReadTime < prev.readTime ||
		d.WriteTime < prev.writeTime ||
		d.IoTime < prev.ioTime ||
		d.WeightedIO < prev.weightedIO ||
		d.ReadCount < prev.readCount ||
		d.WriteCount < prev.writeCount
}
