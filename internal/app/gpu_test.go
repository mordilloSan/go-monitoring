...
func TestParseNvtopData(t *testing.T) {
	input, err := os.ReadFile("testdata/nvtop.json")
...
echo "[]"`
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			priorities := gm.resolveAutoCollectorPriority(caps)
...
func TestNewGPUManagerPriorityNvtopFallback(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("PATH", dir)
	t.Setenv("GPU_COLLECTOR", "nvtop,nvidia-smi")
...
func TestNewGPUManagerPriorityMixedCollectors(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("PATH", dir)
	t.Setenv("GPU_COLLECTOR", "intel_gpu_top,rocm-smi")
...
func TestNewGPUManagerPriorityNvmlFallbackToNvidiaSmi(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("PATH", dir)
	t.Setenv("GPU_COLLECTOR", "nvml,nvidia-smi")
