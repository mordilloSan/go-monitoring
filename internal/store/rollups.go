...
func loadHistoryJSON(tx *sql.Tx, table, resolution string, capturedAfter, capturedAtOrBefore int64) ([]string, error) {
	...
	`, table), resolution, capturedAfter, capturedAtOrBefore)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func averageSystemStatsJSON(items []string) (*system.Stats, error) {
	...
	if count > 0 {
		...
		sum.DiskIO[0] /= uint64(count)
		sum.DiskIO[1] /= uint64(count)
		...
		sum.Bandwidth[0] /= uint64(count)
		sum.Bandwidth[1] /= uint64(count)
